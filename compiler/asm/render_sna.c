...
uint32_t render_sna(compile_ctx_t *ctx, char **dest_buf) {
  ...
  if (ctx->sna_pc_addr != -1) {
    if (ctx->sna_pc_addr < SNA_SNAPSHOT_ROM_SIZE) {
      bool saved_verbose = ctx->verbose_error;
      ctx->verbose_error = false;
      report_warning(ctx, "snapshot PC value refers to ROM area");
      ctx->verbose_error = saved_verbose;
    }

    pc_value = ctx->sna_pc_addr;
  } else {
  ...
  foreach (dc, ctx->sections) {
    ...
    if (section->start < SNA_SNAPSHOT_ROM_SIZE) {
      ctx->verbose_error = false;
      report_error(ctx, "can't place section '%s' with start address 0x%x into RAM snapshot",
        section->name, section->start);
    }

    if (section->start - SNA_SNAPSHOT_ROM_SIZE + section->content->len > SNA_SNAPSHOT_RAM_SIZE) {
      ctx->verbose_error = false;
      report_error(ctx, "can't place section '%s' with size %d into RAM snapshot due RAM size overflow",
  ...
  if (!ctx->sna_generic) {
    ...
    ram_snapshot_start[ZX_SYSVARS_START + 0xb3 - SNA_SNAPSHOT_ROM_SIZE] =
      (ctx->sna_ramtop >> 8) & 0xff;

    addr = ctx->sna_ramtop + 1 - sizeof(zx_stack_data);
    ...
    ctx->sna_ramtop = ctx->sna_ramtop + 1 - sizeof(zx_stack_data) - sizeof(uint16_t);
  }

  header->sp = ctx->sna_ramtop;

  // inject PC into top of stack
  ram_snapshot_start[ctx->sna_ramtop + 1 - SNA_SNAPSHOT_ROM_SIZE] = (pc_value >> 8) & 0xff;
  ram_snapshot_start[ctx->sna_ramtop - SNA_SNAPSHOT_ROM_SIZE] = pc_value & 0xff;
