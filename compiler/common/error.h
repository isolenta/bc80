...
typedef int (*error_callback_type) (const char *message, const char *filename, int line, int pos);

// the same as error_callback_type but for non-critical warnings
typedef void (*warning_callback_type) (const char *message, const char *filename, int line, int pos);

extern void set_error_context(error_callback_type error_cb, warning_callback_type warning_cb, jmp_buf *error_env);

extern void generic_report_error(const char *filename, int line, int pos, char *fmt, ...);
extern void generic_report_warning(const char *filename, int line, int pos, char *fmt, ...);
