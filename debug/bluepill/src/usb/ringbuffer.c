...
// head == tail - empty! So, buffer can't store more than RBSIZE-1 bytes of
// data!
...
static int datalen() {
  if (tail >= head)
    ...
    return (RBSIZE - head + tail);
}
static int restlen() { return (RBSIZE - 1 - datalen()); }

static void mcpy(char *targ, const char *src, int l) {
  while (l--) *targ++ = *src++;
}

__attribute__((always_inline)) static inline void incr(volatile int *what,
                                                       int n) {
  *what += n;
